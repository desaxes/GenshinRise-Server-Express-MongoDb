...
export const RollRepository = {
    ...
    async getWeaponRollsForBanner(finalConditions: any, limit: number, offset: number, year: number, lmonth: number, lday: number, hmonth: number, hday: number) {
        // const rollCounter = (await erollDb.find<newRollType>(finalConditions).toArray()).length
        const rolls = await wrollDb.find({ year: year, month: { $in: [lmonth, hmonth] }, stars: { $in: [4, 5] } }).sort({ stars: -1, isChar: -1, rewardId: -1 }).skip(offset).limit(limit).toArray()
        const rollCounter = await wrollDb.find({ year: year, month: { $in: [lmonth, hmonth] }, stars: { $in: [3, 4, 5] } }).sort({ stars: -1, isChar: -1, rewardId: -1 }).skip(offset).limit(limit).toArray()
        let sort
        let sortCounter
        if (lmonth === hmonth) {
            sort = rolls.filter(e => e.day >= lday && e.day <= hday)
