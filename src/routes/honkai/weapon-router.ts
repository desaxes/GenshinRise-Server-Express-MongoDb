...
export const honkaiWeaponRouter = () => {
    ...
    router.get('/:id',
        async (req: RequestWithParams<URIParamsModel>, res: Response<newWeapon | null>) => {
            try {
                const weapon = await honkaiWeaponService.getWeaponById(req.params.id)
    ...
    router.post('/',
        async (req: RequestWithBody<weaponType>, res: Response<InsertOneResult | { message: string }>) => {
            try {
                let files = req.files
                const newWeapon = await honkaiWeaponService.createWeapon(req.body, files)
