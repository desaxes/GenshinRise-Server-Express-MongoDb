...
export const zzzRiseRouter = () => {
    const router = express.Router()
    router.get('/',
        async (req: RequestWithQuery<CharQueryModel>, res: Response<any>) => {
            try {
                let chars = await zzzCharacterService.getCharsFromRise(req.query)
    ...
    router.get('/:id',
        async (req: RequestWithParams<URIParamsModel>, res: Response<newChar | null>) => {
            try {
                const char = await zzzCharacterService.getCharByIdFromRise(req.params.id)
    ...
    router.post('/',
        async (req: RequestWithBody<newChar>, res: Response<InsertOneResult | { message: string }>) => {
            try {
                const newChar = await zzzCharacterService.addCharToRise(req.body)
    ...
    router.put('/',
        async (req: RequestWithBody<updateChar>, res: Response<UpdateResult | { message: string }>) => {
            try {
                const updated = await zzzCharacterService.updateCharToRise(req.body)
    ...
    router.delete('/:id',
        async (req: RequestWithParams<URIParamsModel>, res: Response<{}>) => {
            try {
                await zzzCharacterService.removeCharFromRise(req.params.id)
