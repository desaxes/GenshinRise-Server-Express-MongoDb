...
export const zzzBannerRouter = () => {
    const router = express.Router()
    router.get('/',
        async (req: RequestWithQuery<RollQueryModel>, res: Response<any>) => {
            try {
                let banners = await zzzBannerService.getAllBanners(req.query)
    ...
    router.get('/:id',
        async (req: Request, res: Response<any>) => {
            try {
                let banners = await zzzBannerService.getBanner(+req.params.id)
                res.json(banners).status(200)
            }
            catch (e) {
                res.sendStatus(500)
    ...
    router.post('/',
        async (req: RequestWithBody<any>, res: Response<InsertOneResult | { message: string }>) => {
            try {
                let files = req.files
                const newBanner = await zzzBannerService.createBanner(req.body, files)
